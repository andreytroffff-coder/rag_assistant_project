...
- В API `/ask` пока возвращается только `answer`, без списка `sources`.
- В API `/reindex` нужно выровнять путь документов с CLI.
- Нет автоматических тестов.
- Chunking пока режет текст по символам, а не по словам или предложениям.
- Нужно следить, чтобы `vector_size` совпадал с выбранной embedding model.

## Roadmap

- Вернуть `sources` в ответе API `/ask`.
- Вынести путь к документам в `config.py`.
- Добавить тесты для `ingest`, `retriever`, `llm`, `api`.
- Улучшить chunking, чтобы не резать текст посередине слов.
- Добавить Dockerfile для API-сервиса.
- Добавить CI-проверки: lint + tests.
